...
contract UserContract {
    ...
    constructor(address _addressBook, address _userAddress) public{ //This function is only used for user registration in the system
        ...
        require (msg.sender == adb.getAddress("RegContract"), 'This contract only can be deployed by Register Contract');
        availableToken = 0;
        active = false;
...
/*    function updateAvailableToken(int _token) public {
    ...
    //This function is called when registration smart contract validates the user's registration request and wants to add th new service in the user's service list.
        //AddressBook adb = AddressBook(addressBook);
        //require(msg.sender == adb.getAddress("RegContract"), 'you can not call this function');
        userRegisteredServices memory _temp;
        _temp = userRegisteredServices(_providerCode, _serviceCode, (block.timestamp + _dateOrTime * 86400), _dateOrTime, true, _serviceType);
        //if(_serviceType == 1)
    ...
    //function Activate(bool _flag) public {
        //AddressBook adb = AddressBook(addressBook);
       // require(msg.sender == adb.getAddress("ACManager"), 'you can not call this function');
       // active = _flag;
    //}
    ...
    function payToken(int _token) public {
        //AddressBook adb = AddressBook(addressBook);
        //require(msg.sender == adb.getAddress("ACManager") || msg.sender == adb.getAddress("RegContract"), "This  function is restricted");

...
contract ServiceProviderContract is Owned {
    ...
    struct Services {
        int serviceCode;
        int serviceType; //if serviceType ==1, the service is prePaid, if it is 2, the service is Pay-as you go
        ...
        int operatorShare; // percentage_ between 0-10
    }

    mapping(int => mapping(int => bool)) policyTitle; // This parameter can be added by sp themselves

    function addPolicy(int _title, int _serviceCode) public {
    ...
    function addNewService(int _serviceType, int _serviceCostforUser, int _operatorFee, int _costForUser, int _minPrice, int _operatorShare) public {
        //AddressBook adb = AddressBook(addressBook);
        //require(msg.sender == adb.getAddress("regContract"), "This function is restricted");
        serviceCode +=1;
    ...
    function chargeMyWallet(int Token) public {
    ...
    function addToken(int Token) public {
        ...
        //AddressBook adb = AddressBook(addressBook);
        //require(msg.sender == adb.getAddress("RegContract"));
        availableToken += Token;
    }
    ...
    function payToNP(int Token) public {
        ...
        //AddressBook adb = AddressBook(addressBook);
        //require(msg.sender == adb.getAddress("ACManager"));
        availableToken -= Token;
    }
